...
def carregar_dados():
    url = "http://127.0.0.1:5000/dados_vwsomelier?data_inicial=2023-01-01&data_final=2025-12-31&pagina=1&limite=500000"  # Alterar para o seu endpoint real
...
def exibir_tabela(df_filtrado):
    ...
    df_resumo['Total Vendido'] = df_resumo['Total Vendido'].apply(lambda x: f"{x:,.0f}".replace(',','.'))
...
def exibir_grafico_top_produtos(df, periodo_inicial, periodo_final):
    ...
    fig.update_layout(title_font_size=20, xaxis_title_font_size=16, yaxis_title_font_size=16,
                      xaxis_tickfont_size=10, yaxis_tickfont_size=14, xaxis_tickangle=-45)
